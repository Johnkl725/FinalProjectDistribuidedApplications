...
export class RentInsuranceService {
  ...
  async createPolicy(data: CreateRentInsuranceDTO): Promise<RentInsurancePolicy> {
    ...
    const rentInsurance = InsuranceFactory.createInsurance('rent', insuranceData);
  ...
  async getQuote(data: QuoteRentInsuranceDTO): Promise<{ premium: number; details: any }> {
    const insuranceData: RentInsuranceData = {
      userId: 0,
      coverageAmount: data.coverage_amount,
      startDate: new Date(data.start_date),
  ...
  async getPolicyById(policyId: number, userId?: number): Promise<RentInsurancePolicy> {
  ...
  async getPolicyByNumber(policyNumber: string, userId?: number): Promise<RentInsurancePolicy> {
  ...
  async cancelPolicy(policyId: number, userId?: number): Promise<RentInsurancePolicy> {
    if (userId) {
      const policy = await this.repository.findById(policyId);
